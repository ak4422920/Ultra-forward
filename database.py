...
class Database:
    ...
    def __init__(self, uri, database_name):
        ...
        self.live = self.db.live_tasks # New: For Live Forwarding (Auto-Post)
    ...
    async def get_configs(self, id):
        default = {
            'caption': None,
            'duplicate': True,
            'forward_tag': False, # For Forward Tag Remover
            'file_size': 0,
            'size_limit': None,
            'extension': None,
            'keywords': None,
    ...
    async def get_filters(self, user_id):
       ...
       filter_dict = config['filters']
       for k, v in filter_dict.items():
          if v == False:
            filters.append(str(k))
       return filters
              
    async def add_frwd(self, user_id, data=None):
       # Feature 3: Auto-Resume (Storing task data)
       payload = {'user_id': int(user_id)}
       if data: payload.update(data)
       return await self.nfy.insert_one(payload)
    ...
    # New Functions for Feature 1: Live Forwarding
    async def add_live_task(self, user_id, source, targets):
        return await self.live.update_one(
            {'user_id': int(user_id), 'source': source},
            {'$set': {'targets': targets}},
            upsert=True
        )

    async def get_live_tasks(self, source=None):
        query = {'source': source} if source else {}
        return self.live.find(query)

    async def remove_live_task(self, user_id, source):
        return await self.live.delete_one({'user_id': int(user_id), 'source': source})
